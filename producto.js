...
async function cargarComentarios(id) {
  ...
  querySnapshot.forEach((doc) => {
    const data = doc.data();
    count++;
    totalEstrellas += data.estrellas || 0;

    const div = document.createElement("div");
    div.className = "comentario";
    div.innerHTML = `
      <p><strong>${data.usuario || "Anónimo"}</strong> 
      ${"⭐".repeat(data.estrellas || 0)}</p>
      <p>${data.texto}</p>
  ...
  const promedioDiv = document.getElementById("promedio-estrellas");
  if (count > 0) {
    const promedio = (totalEstrellas / count).toFixed(1);
    promedioDiv.innerHTML = `⭐ ${promedio}/5 (${count} reseñas)`;
  } else {
    promedioDiv.innerHTML = "Sin reseñas aún";
  }

  activarAccionesComentarios(id);
}

import { updateDoc, deleteDoc } from "./Firebase.js";

function activarAccionesComentarios(productoId) {
  document.querySelectorAll(".btn-borrar").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const id = btn.getAttribute("data-id");
      if (confirm("¿Seguro que quieres borrar este comentario?")) {
        await deleteDoc(doc(db, "productos", productoId, "comentarios", id));
  ...
  document.querySelectorAll(".btn-editar").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (nuevoTexto) {
        await updateDoc(doc(db, "productos", productoId, "comentarios", id), {
          texto: nuevoTexto
        });
        cargarComentarios(productoId);
      }
